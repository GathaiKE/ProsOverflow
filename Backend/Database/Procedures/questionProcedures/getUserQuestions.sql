...
create or alter procedure getUserQuestions(@user_id VARCHAR(100),@pageSize INT,@pageNumber int)
as
begin
    ...
    WHERE q.RowNumber BETWEEN (@PageSize * (@PageNumber - 1) + 1) AND (@PageSize * @PageNumber) and q.user_id=@user_id
end